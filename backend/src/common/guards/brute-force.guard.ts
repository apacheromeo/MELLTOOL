...
export class BruteForceGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      // If Redis is unavailable or any error occurs, log warning and allow request
      // This prevents Redis issues from blocking legitimate login attempts
      if (error instanceof HttpException) {
        // Re-throw rate limit exceptions
        throw error;
      }
      this.logger.warn(
        `Brute force check failed (Redis unavailable), allowing request: ${error.message}`,
      );
    }
  ...
  async recordFailedAttempt(request: Request, email?: string): Promise<void> {
    try {
      ...
      // If Redis is unavailable, log warning but don't fail the operation
      this.logger.warn(
        `Failed to record login attempt (Redis unavailable): ${error.message}`,
      );
    }
  }
  ...
  async clearAttempts(request: Request, email?: string): Promise<void> {
    try {
      ...
      // If Redis is unavailable, log warning but don't fail the operation
      this.logger.warn(
        `Failed to clear login attempts (Redis unavailable): ${error.message}`,
      );
    }
  }
